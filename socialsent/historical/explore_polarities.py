...
def build_timeseries(raw=False, suffix="-test", years=constants.YEARS):
    ...
    for year in years:
        ...
        for i, (w, p) in enumerate(sorted(list(polarities.items()), key=itemgetter(1))):
            if not raw:
                polarities[w] = i / float(len(polarities))
            else:
                polarities[w] = p

        for w, p in polarities.items():
            timeseries[w].append(p)
    for w in list(timeseries.keys()):
...
def build_boot_timeseries(suffix="-test", years=constants.YEARS):
    ...
    for w in list(mean_timeseries.keys()):
...
def get_boot_meanseries(suffix="-test", years=constants.YEARS):
    ...
    for year in years:
        ...
        for polarities in polarities_list:
            year_means.append(np.mean(list(polarities.values())))
...
def get_boot_meanseriess(suffix="-test", years=constants.YEARS):
    ...
    for year in years:
        ...
        for i, polarities in enumerate(polarities_list):
            mean_timeseriess[i].append(np.mean(list(polarities.values())))
...
def build_boot_ztimeseries(suffix="-test", years=constants.YEARS):
    ...
    for year in years:
        polarities_list = util.load_pickle(constants.POLARITIES + year + suffix + '.pkl')
        means = [np.mean(list(polarities.values())) for polarities in polarities_list]
        stds = [np.std(list(polarities.values())) for polarities in polarities_list]
...
def zscore(dictseries, years=list(range(1850, 2000, 10))):
    timeseries = defaultdict(list)
    yearseries = get_yearseries(dictseries, years)
    for year in years:
        year_mean = np.mean(list(yearseries[year].values()))
        year_std = np.std(list(yearseries[year].values()))
        for word in yearseries[year]:
            timeseries[word].append((dictseries[word][year] - year_mean)
                    / year_std)
    return timeseries

def get_yearseries(dictseries, years=list(range(1850, 2000, 10))):
    yearseries = {}
    for year in years:
...
def plot_timeseries(timeseries, w):
    plt.figure()
    plt.plot(list(map(int, constants.YEARS)), timeseries[w])
    plt.title(w)
    plt.xlabel('date')
    plt.ylabel('polarity')
    plt.show()
